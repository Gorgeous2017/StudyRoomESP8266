...
//			printmac(sniffer->buf, 4); // 目的地址
//			printmac(sniffer->buf, 10); // 源地址
//			os_printf("\n");
		//uint8 sniffer_test[3]={0};
		//os_memset(sniffer_test, sniffer->buf[4 + 3], 3);
		static uint8 j = 1;
		os_memcpy( (msg_buf+j), (sniffer->buf)+7, 3);
		j += 3;



//		os_printf("%02X",msg_buf[0]);
//		printmac(msg_buf, 1);
//		printmac(msg_buf, 7);
//		printmac(msg_buf, 13);
//		//printmac(msg_buf, 17);
//		os_printf("\n");
		...
		//uart0_tx_buffer((sniffer->buf)+7, 3);

		#endif
			//send2server(&sniffer->buf[4],6);
		// 判断client
        for (i=0; i<6; i++) if (sniffer->buf[i+4] != client[i]) return;
        //printmac(sniffer->buf, 4);

//        os_printf("\r");
//        os_printf("\trssi:%d\r", sniffer->rx_ctrl.rssi);
//        os_printf("\tchannel:%d\r", sniffer->rx_ctrl.channel);
//        os_printf("\trate:%d\r\n", sniffer->rx_ctrl.rate);
//        os_printf("\tsig_mode:%d\r",sniffer->rx_ctrl.sig_mode);
