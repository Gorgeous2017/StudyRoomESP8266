...
tcp_client_recv(void *arg, char *pdata, unsigned short len) {
	...
	os_printf("lenth:%d\r\n", len);
	//os_printf("data:%s\r\n", pdata);
	os_printf("data[0]=%d,data[1]=%d,data[2]=%d,data[3]=%d,data[4]=%d,\r\n", pdata[0],pdata[1],pdata[2],pdata[3],pdata[4]);
//	uint8 buff[10] = {0};
//	os_memcpy(buff,pdata,len);
	//os_printf("fuuuuuuuckkkkk\n");
	uart1_tx_buffer((uint8 *)pdata,len);


	//addRoomNum(pdta,ROOM_NO);
...
send2server (uint8 *pdata,uint8 len) {
